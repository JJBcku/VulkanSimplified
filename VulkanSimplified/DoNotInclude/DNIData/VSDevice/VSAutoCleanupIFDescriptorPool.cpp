...
namespace VulkanSimplified
{
	...
	void AutoCleanupIFDescriptorPool::WriteDescriptorSetCombined2DArrayTextureSamplerBindings(
		const std::vector<DescriptorSetCombined2DArrayTextureSamplerWriteDataInternal>& combined2DArrayTextureSamplers)
	{
		if (combined2DArrayTextureSamplers.empty())
			return;

		std::vector<VkWriteDescriptorSet> descriptorWriteDataList;
		descriptorWriteDataList.resize(combined2DArrayTextureSamplers.size());

		for (size_t i = 0; i < combined2DArrayTextureSamplers.size(); ++i)
		{
			auto& inData = combined2DArrayTextureSamplers[i];
			auto& writeData = descriptorWriteDataList[i];
	...
	std::vector<bool> AutoCleanupIFDescriptorPool::FreeDescriptorSets(const std::vector<IDObject<AutoCleanupDescriptorSet>>& descriptorSetsIDs, bool throwOnIDNotFound)
	{
		...
		for (size_t i = 0; i < descriptorSetsIDs.size(); ++i)
		{
			...
			if (found)
			{
				descriptorSetList.push_back(_descriptorSetList.GetObject(descriptorSetsIDs[i]).GetDescriptorSet());
